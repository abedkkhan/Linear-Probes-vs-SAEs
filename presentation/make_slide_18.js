// Slide 18 — Linear Probe vs SAE · Head-to-Head
// Theme: Claude Code UI — matches slides 1, 2, 15, 16, 17 exactly.
...
(async () => {
  ...
  pres.title  = "Probe vs SAE — Head-to-Head";
  ...
  // ------------------------------------------------------------------
  // Stat row — three comparison callouts
  // ------------------------------------------------------------------
  const STAT_Y    = 2.05;
  const STAT_H    = 1.20;
  const STAT_GAP  = 0.35;
  ...
  const stats = [
    {
      big: "92.0%",
      label: "LINEAR PROBE",
      sub: "1 supervised direction across 2,304 dims",
    },
    {
      big: "89.8%",
      label: "SAE · TOP-100",
      sub: "~100 unsupervised features, no labels used",
    },
    {
      big: "−2.2 pp",
      label: "GAP · PROBE vs SAE",
      sub: "probe wins by a narrow margin on accuracy",
    },
  ];
  ...
  // ------------------------------------------------------------------
  // Plot row — three comparison figures
  // ------------------------------------------------------------------
  ...
  const plots = [
    {
      file: path.join(PLOTS, "accuracy_comparison_bar.png"),
      cap: "Accuracy side-by-side across configurations",
    },
    {
      file: path.join(PLOTS, "sae_topk_vs_probe.png"),
      cap: "SAE accuracy curve approaches the probe baseline",
    },
    {
      file: path.join(PLOTS, "f1_auc_comparison.png"),
      cap: "F1 and AUC — same gap, same shape",
    },
  ];
  ...
  slide.addText(
    "The probe wins on accuracy by ~2 pp; the SAE wins on interpretability — each feature can be named, ranked, and inspected.",
    {
  ...
  await pres.writeFile({ fileName: "slide_18.pptx" });
  console.log("Saved slide_18.pptx");
})();
