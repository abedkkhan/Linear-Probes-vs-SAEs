...
All checks passed. Shapes, dtypes, NaN/inf, label alignment and label distributions are exactly as expected, and the per-class activation difference is clearly non-zero on all three splits (0.20 on train, 0.17 on val, 0.24 on test) — meaning the pipeline produces vectors whose average values genuinely differ between positive and negative sentences, which is the minimum precondition for a linear probe to be able to learn anything. One mild observation worth noting: the validation split has a smaller standard deviation (4.64) than train and test (~10.0). The reason is structural to SST-2 itself — our train and test were carved from the original SST-2 train split, which contains a mix of full sentences AND short sub-phrases from the parse tree, while our val came from the original SST-2 validation split, which contains only full sentences. Full sentences produce somewhat different activation distributions than 3-word fragments, hence the lower variance. This is a known property of SST-2's construction, not a bug in our pipeline, and it does not affect the validity of the experiment.
