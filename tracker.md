...
**Quick aside on what "std" or "variance" means:** the standard deviation (std) of a dataset is just a number that tells you how spread out the values are around their average. A low std means the numbers are all clustered close to the mean; a high std means they are scattered far above and far below it. Variance is the same idea, just squared. So when we say the val split has "lower variance" than the train split, we are simply saying the activation numbers in val sit closer to their average — they are more uniform, less spread out — while the train and test activations include some bigger swings in either direction. In our case this happened because val contains only full sentences, while train and test contain a mixture of full sentences and short fragments, and that mixture naturally produces a wider range of activation values.
